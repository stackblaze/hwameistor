...
func SnapshotName(pvcName, runID string) string {
	return fmt.Sprintf("%s%s%s", pvcName, BackupJobSnapshotInfix, runID)
}

func TempPVCName(pvcName, runID string) string {
	return fmt.Sprintf("%s%s%s", pvcName, BackupJobTempPVCInfix, runID)
}
...
func RestoreJobName(restoreName string) string {
	return restoreName + RestoreJobSuffix
}
...
func commonLabels(cfg RuntimeConfig) map[string]string {
	return map[string]string{
		...
		BackupOwnerPVCNamespaceLabel: cfg.PVCNamespace,
		BackupOwnerPVCNameLabel:      cfg.PVCName,
	}
}

func ownerRef(cfg RuntimeConfig) metav1.OwnerReference {
	return metav1.OwnerReference{
		APIVersion:         hwameistorv1alpha1.SchemeGroupVersion.String(),
		Kind:               "PVCBackup",
