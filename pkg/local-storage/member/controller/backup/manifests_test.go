...
func baseCfg() RuntimeConfig {
	return RuntimeConfig{
		...
		ServiceAccountName:       "hwameistor-backup-mover",
	}
}
...
func TestNames_AreDeterministicAndDistinct(t *testing.T) {
	...
	if !strings.HasSuffix(restore, RestoreJobSuffix) {
...
func TestSnapshot_HasOwnerRefAndLabels(t *testing.T) {
	...
	if snap.Spec.VolumeSnapshotClassName == nil || *snap.Spec.VolumeSnapshotClassName != "hwameistor-storage-lvm-snapshot" {
		t.Fatalf("snapshot class not propagated")
	}
	if snap.Labels[BackupManagedByLabel] != BackupManagedByValue {
...
func TestMoverEnv_SecretKeysMatchConstants(t *testing.T) {
	...
	want := map[string]string{
		"RESTIC_PASSWORD":       ResticPasswordSecretKey,
		"AWS_ACCESS_KEY_ID":     S3AccessKeyIDSecretKey,
		"AWS_SECRET_ACCESS_KEY": S3SecretKeySecretKey,
	}
...
func TestMoverEnv_RetentionFlagsPropagated(t *testing.T) {
	cfg := baseCfg()
	cfg.Retention = hwameistorv1alpha1.RetentionSpec{
		KeepLast:    3,
		KeepDaily:   7,
		KeepMonthly: 0,
	}
