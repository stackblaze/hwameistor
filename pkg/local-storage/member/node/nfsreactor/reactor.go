...
//  2. Writes the EXPORT{} block to <ExportRoot>/ganesha.d/<id>.conf and
//     sends SIGHUP to ganesha; ganesha re-parses its %dir-included
//     config directory and diffs add/remove/update without dropping
//     live NFS connections.
...
type Options struct {
	...
	PodIP        string
	PodNamespace string
	// ExportRoot: shared dir between reactor and ganesha containers.
	// The reactor mounts LVs at <ExportRoot>/<uid> and writes per-export
	// ganesha config files at <ExportRoot>/ganesha.d/<exportID>.conf.
	// Ganesha's top-level config must %dir-include <ExportRoot>/ganesha.d.
	ExportRoot string
	// GaneshaPidFile: ganesha writes its pid here; the reactor reads it
	// to send SIGHUP on config change.
	GaneshaPidFile string
...
func New(opts Options) *Reactor {
	return &Reactor{
		...
		ganesha: NewGaneshaClient(filepath.Join(opts.ExportRoot, "ganesha.d"), opts.GaneshaPidFile),
...
func (r *Reactor) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	// GaneshaClient writes its own config file under <ExportRoot>/ganesha.d
	// and sends SIGHUP; the `path` arg is kept for interface symmetry
	// with historical DBus implementations and is unused.
	if err := r.ganesha.AddExport(exportID, "", cfg); err != nil {
